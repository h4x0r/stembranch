...
\zh{八字}'s algorithmic simplification has a well-known information-theoretic
...
The classical resolution is not denial but \emph{stratification}.
The ``Cosmic Trinity'' model (\zh{天地人}---Heaven, Earth, Human) holds
that the birth chart constitutes only one-third of destiny; the remaining
two-thirds come from geography (\zh{風水}, \zh{地利}) and individual
cultivation (\zh{修行}, \zh{人和}).
The well-known \zh{同命不同運} (``same chart, different fortune'')
principle is illustrated by the Emperor-and-Beekeeper fable: both
share identical \zh{八字}, but one commands an empire and the other
commands a hive.  The chart describes the \emph{pattern}, not the
\emph{scale}---a framework that Geng~Li's ethnography of modern
practitioners confirms remains central to professional
practice~\cite{geng_li_2019}.
Lisa Raphals has argued that this allowance for human agency
distinguishes Chinese fate calculation from Greek deterministic
fate~\cite{raphals_2003}.
...
This is not a deficiency to be apologized for but a deliberate
architectural trade-off: what was lost in astronomical resolution was
gained in computational permanence and interpretive
flexibility~\cite{smith_fortune_2019, ho_peng_yoke_2003}.
