...
% Companion paper — XeLaTeX + xeCJK, two-column
% =============================================================================
...
% CJK helper (pass-through; xeCJK handles CJK natively)
\newcommand{\zh}[1]{#1}

% gensymb compatibility (XeLaTeX uses Unicode degree sign directly)
\providecommand{\degree}{\ensuremath{°}}
...
(\textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i}, \zh{三辰通載}); equatorial sidereal in the Yuan (\textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n}, \zh{鄭氏星案}); a novel
quasi-ecliptic sidereal hybrid in the Ming (\textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng}, \zh{果老星宗}); and ecliptic tropical
...
The paper contributes: (1)~computational demonstration of the apsidal identification
using JPL~DE431 ephemeris data; (2)~the first rigorous definition of the
...
\begin{enumerate}[leftmargin=*, label=(\arabic*)]
  \item \textbf{Song ecliptic sidereal.}  The \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} (\zh{三辰通載}, ``Comprehensive Record of the Three Luminaries,'' c.~11th century~CE)
    presents Ketu positions in a coordinate frame that is recognisably ecliptic
    and sidereal, close to the Indian original.
  \item \textbf{Yuan equatorial sidereal.}  The \textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n} (\zh{鄭氏星案}, ``Zheng's Stellar Cases,'' late Yuan, c.~14th
  ...
  \item \textbf{Ming quasi-ecliptic sidereal.}  The \textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng} (\zh{果老星宗}, ``Star-School of the Elder of the Fruit,'' Ming dynasty,
    c.~15th--16th century) introduces what we term the \textit{sihuangdao} (\zh{似黃道}, quasi-ecliptic)
...
Niu Weixing's landmark 1994 analysis of the \textit{Qiy\`ao r\`angz\=ai ju\'e} (\zh{七曜攘災訣}, ``Formulae for Averting Disasters Caused by the Seven Luminaries,'' 806~CE), a Tang
...
Ketu as apogee was thoroughly assimilated into \textit{qizheng siyu} (\zh{七政四餘}, ``Seven Governors and Four Remainders'') practice, where it
joined Rahu, the Purple Star (\textit{Ziq\`{\i}}, \zh{紫氣}), and the Monthly Comet
(\textit{Yu\`{e}b\`{e}i}, \zh{月孛}) as the four ``remainders''---invisible bodies that governed fate
...
The Ming quasi-ecliptic system is the prime instance.  It preserves ecliptic-style
...
routinely inherits the Ming quasi-ecliptic frame without labelling it as such,
...
demonstration using modern ephemeris data.  For the computational component
we use the JPL Development Ephemeris DE431 (accessed via the Swiss Ephemeris
library, version~2.10).  This allows us to ask a precise empirical
...
the approach used by Niu Weixing \cite{niu1995ketu} for the \textit{Qiy\`ao r\`angz\=ai ju\'e} and
...
software.  Section~\ref{sec:computation} presents the quantitative demonstration.
...
occur in Tang dynasty China, when Ketu moved from node to apogee.
...
sections.  The contrast is arresting.  In the Chinese \textit{qizheng siyu}
system, Rahu retains its Indian identity as the ascending
node, but Ketu has been detached from the descending node
...
no direct causal relationship to eclipses.  A new body, \textit{Yu\`eb\`ei}
(``lunar comet''), has been invented to track the
mean apogee, and a fourth fictitious point, \textit{Ziq\`{\i}}
...
The pivotal document is the \textit{Jiuzhi li}
(\textit{Navagraha-karana}, ``Calendar of the Nine Seizers''), translated
in 718~CE by Gautama Siddh\=artha, an astronomer of
...
(\zh{太史局}, \textit{T\`aish\v{\i}j\'u}) and brought to his translation both fluency in Sanskrit
...
\subsection*{The \textit{Qiy\`ao r\`angz\=ai ju\'e} and the Apogee Identification (806~CE)}

The second key Tang text is the \textit{Qiy\`ao r\`angz\=ai ju\'e}
(``Formulae for Averting Disasters Caused by the Seven
Luminaries''), a Buddhist astrological manual compiled by a
Br\=ahma\d{n}a priest from western India sometime after 806~CE, the
epoch year (Year~1 of the Yuanhe reign period) embedded in its
...
translation---Ketu had already ceased to designate the descending
...
sidereal system for Chinese \textit{qizheng siyu} astrology would finally be made.
...
li}: foreign names and methods are adopted wholesale, with Rahu
and Ketu entering Chinese as phonetic loan words for Indian
concepts.  Mode~2 (reidentification) is already under way: the name
Ketu has been retained but silently detached from its original
referent.
...
Ketu as apogee---that defines the mature \textit{qizheng siyu} system and
persists in software to this day.

% === §4 The Song System =======================================================
\section{Ecliptic Sidereal: The Song System (\textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i})}
\label{sec:song}
% ~1,200 words — Task 8
...
correction of the Qing, there existed a Chinese \textit{qizheng siyu} practice that
looked, structurally, very much like what a medieval Indian astrologer would
recognize.  The \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} is our primary evidence for
this baseline state.

\subsection*{The \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} as Earliest Codification}

The \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} is the earliest extant Chinese text to codify the full
\textit{qizheng siyu} system in a form recognisably continuous with its Indian
antecedents.\footnote{Direct access to the \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} text remains limited;
...
seven visible planets (\textit{q\={\i}zh\`eng}, \zh{七政}: Sun, Moon, Mercury, Venus, Mars, Jupiter,
Saturn) together with the four ``remainders'' (\textit{s\`{\i}y\'u}, \zh{四餘}) into a unified
predictive scheme.

What makes the \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} a baseline rather than merely an early text
...
The \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} employs what we designate the \textit{hu\'{a}ngd\`ao h\'{e}ngx\=ing zh\`{\i}} (\zh{黃道恆星製}, ``ecliptic sidereal
...
intermediaries.  Crucially, no Five-Phase overlay is present.
...
a native Chinese institution; its presence in the \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} is a
...
in the \textit{D\`ongw\=ei b\`aili\`u xi\`an} shows closer affinity with the Indian
...
The most consequential development in the \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} for the purposes
of this paper is the crystallization of the \textit{s\`{\i}y\'u} as a canonical set.
By the Song period the four fictitious bodies have settled into their
classical configuration: Rahu (ascending node), Ketu,
\textit{Yu\`eb\`ei} (``Lunar Comet,'' mean apogee),
and \textit{Ziq\`{\i}} (``Purple Vapour,'' a 28-year
intercalary point).

The identity of Ketu in this period is significant.  As noted in
...
tension---Ketu as osculating apogee and \textit{Yu\`eb\`ei} as mean
apogee simultaneously present in the same system---is therefore a
structural feature of \textit{qizheng siyu} from its earliest codification,
...
The \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} system is significant precisely as a \emph{baseline}.
...
This makes the \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} methodologically invaluable.  The
...
Section~\ref{sec:intro}, the \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} represents the terminal
...
% === §5 The Yuan Reform =======================================================
\section{Equatorial Sidereal: The Yuan Reform (\textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n})}
\label{sec:yuan}
% ~1,500 words — Task 8
...
the entire history of Chinese \textit{qizheng siyu}.

\subsection*{Historical Context: Guo Shoujing and the \textit{Shoushi li}}
...
Chinese history.  The resulting \textit{Shoushi li}
(``Season-Granting Calendar,'' 1280) achieved solar-year
...
The \textit{Shoushi li} also solved, for the first time in Chinese astronomy,
...
conversion algorithm demonstrates that the \textit{Shoushi li} employed a
...
influenced the \textit{qizheng siyu} practitioners who produced the
\textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n} remains an open question; but the institutional and
...
\subsection*{The Text: \textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n}}

The \textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n}
(``Zheng's Stellar Cases'') is a Yuan-dynasty \textit{qizheng siyu} manual
...
equatorial sidereal characterization of the \textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n} awaits full
...
The \textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n} employs the \textit{ch\`{\i}d\`ao h\'{e}ngx\=ing zh\`{\i}} (\zh{赤道恆星製},
``equatorial sidereal system'').  Where the Song \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i}
...
Chinese system of twenty-eight lunar mansions (\textit{xi\`u}).  The twelve zodiacal houses,
...
The \textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n} does not attempt to preserve it.  What survives the
...
to the new equatorial sidereal frame in practice?  The \textit{Shoushi li}'s
...
\textit{qizheng siyu} tables into equatorial coordinates---is mathematically
equivalent.

Whether \textit{qizheng siyu} practitioners had direct access to the
\textit{Shoushi li}'s computational machinery, or worked from
...
is the outcome: the Yuan \textit{qizheng siyu} practitioner no longer
...
\textit{qizheng siyu} positions directly comparable with, and perhaps
...
The Yuan reform touches coordinates but not identity.  The \textit{s\`{\i}y\'u}
continue within the equatorial framework with their Song-period
identifications undisturbed.  Rahu remains the ascending
node.  Ketu remains the osculating lunar apogee.  \textit{Yu\`eb\`ei}
continues to track the mean apogee.
\textit{Ziq\`{\i}} continues its 28-year cycle.

The doubled-apogee tension noted in Section~\ref{sec:song}---two
bodies, Ketu and \textit{Yu\`eb\`ei}, both apsidal, one osculating and
...
the nature of the conversion actually performed.  The quantitative
demonstration in Section~\ref{sec:computation} exploits exactly
this property.
...
preference---a desire to align \textit{qizheng siyu} with the prestige
...
\section{Quasi-Ecliptic Sidereal: The Ming Synthesis (\textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng})}
\label{sec:ming}
% ~1,800 words — Task 9
...
\textit{qizheng siyu} tradition achieves maximum distance from its Indian
...
\textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng} (``Star-School
of the Elder of the Fruit''), the Ming-dynasty canonical text of the
\textit{qizheng siyu} tradition.\footnote{The \textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng} takes its name
...
\textit{qizheng siyu} practice, and its coordinate conventions are therefore
...
\textit{qizheng siyu} scholarship.  Where they agree---and on the coordinate
conventions they agree---we can speak of a Ming consensus.

\subsection*{Coordinate Thread: Defining the Quasi-Ecliptic Sidereal System}

The coordinate system of the Ming texts is the
\textit{s\`{\i} hu\'angd\`ao h\'engx\={\i}ng
zh\`{\i}} (\zh{似黃道恆星製}, ``quasi-ecliptic sidereal system'').  The name encodes
both an assertion and a qualification: the system is \textit{like}
(\textit{s\`{\i}}, \zh{似}) the ecliptic---it maps positions onto zodiac-style signs
...
ecliptic house divisions.  The quasi-ecliptic sidereal system is a genuinely new
coordinate species, not a corruption or a simplification of any
predecessor system.

\subsection*{Identity Thread: The Mature Four Remainders System}

The Ming texts present the most fully elaborated version of the
four-remainder (\textit{s\`{\i}y\'u}) system.  The four bodies are now formally
...
Rahu retains the identity it has held since
...
Ketu is the osculating lunar apogee---the
...
\subsubsection*{The Mean Apogee: \textit{Yu\`eb\`ei}}

\textit{Yu\`eb\`ei} (``lunar comet'' or ``lunar
...
The result is a quantity distinct from Ketu precisely because
it is mean rather than osculating.

\subsubsection*{The Purple Qi: \textit{Ziq\`{\i}}}

\textit{Ziq\`{\i}} (``Purple Qi'') is a fictitious
...
other three remainders, \textit{Ziq\`{\i}} cannot be confirmed or disconfirmed
...
apse line: Ketu (osculating apogee) and \textit{Yu\`eb\`ei} (mean
...
the derivation of \textit{s\`{\i}y\'u} positions from \textit{b\={a}z\`{\i}}
(\zh{八字}, ``eight characters'' or ``four pillars'')
...
\textit{qizheng siyu} system ever achieved from its Indian original,
measured along both the coordinate and identity threads simultaneously.

On the coordinate thread, the quasi-ecliptic sidereal system is the culmination
...
them (\textit{Yu\`eb\`ei} and \textit{Ziq\`{\i}}) without Indian precedent.  The
...
Yet the system has not yet fossilized.  The \textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng} and
\textit{X\={\i}ngxu\'e d\`ach\'eng} both give computational procedures that, however
...
the \textit{qizheng siyu} tradition---more disruptive, in the short term,
...
\textit{Shixian li} (``Calendar of Edicts
...
than those of the Chinese \textit{Shoushi li} over the century of positional
drift that had accumulated since the Yuan reform \cite{jami2015calendar}.

For the \textit{qizheng siyu} system specifically, the reform imposed a
...
\textit{hu\'angd\`ao hu\'{\i}gu\={\i} zh\`{\i}} (\zh{黃道迴歸製},
``ecliptic tropical system'').

\subsection*{Coordinate Thread: The Ironic Completion}

The coordinate history of \textit{qizheng siyu} traces a closed trajectory
...
eleven bodies of the \textit{qizheng siyu} system shift by roughly one
...
\subsection*{Identity Thread: Ketu Restored, \textit{Ziq\`{\i}} Abolished}

The Jesuit reform was no less consequential on the identity thread.
Two changes stand out.

First, Ketu reverted to the descending lunar node.  The
...
Ketu is once again the descending node, its original Indian
...
Second, \textit{Ziq\`{\i}} was abolished entirely.  The purely Chinese
...
An elemental inconsistency followed from the Ketu reidentification.
Within the Five Phases (\textit{w\v{u}x\'{\i}ng})
...
Ketu as apogee and Ketu as descending node occupy
...
``Ketu-north Rahu-south'' / ``Ketu-south Rahu-north'' toggle that appears in modern software
...
\textit{qizheng siyu} convention---ecliptic tropical coordinates, Ketu as
descending node, no \textit{Ziq\`{\i}}---was confirmed as the official
standard.
...
attached to planetary positions---the Five-Phase element assignments,
...
quasi-ecliptic sidereal system: not a mathematical hybrid of coordinate systems
...
irony of \textit{qizheng siyu} history.  The tradition began with ecliptic
...
identifies Ketu as the descending node.  The mainstream
\textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng} tradition (drawing on Ming texts) uses sidereal
coordinates and identifies Ketu as the apogee.  Both claim
...
The process began quietly during the Ming dynasty.  As \textit{qizheng siyu} practice
...
from actual astronomical computation but from the \textit{b\={a}z\`{\i}}
...
authentic tradition, a \textit{qizheng siyu} chart requires computing the ecliptic
...
``looked up'' from tables indexed by the \textit{b\={a}z\`{\i}} itself, making the result a
...
pseudo-planets Rahu and Ketu acquire fixed angular
separations in these tables, destroying the independent variation that is their
only diagnostic value.  Mak calls this the ``\textit{b\={a}z\`{\i}} colonization'' of the
\textit{qizheng siyu} system: the birth chart format survives while its astronomical
...
GitHub)---inherit the Qing-period Jesuit convention and define Ketu as the
\emph{descending} lunar node.  This was, as documented in Section~\ref{sec:jesuit},
...
``Ketu-north Rahu-south'' / ``Ketu-south Rahu-north'' toggle that appears in several software packages.
...
planetary longitudes using the quasi-ecliptic sidereal system
inherited from the Ming \textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng} tradition, while computing the solar and
...
an unintentional archive.  The ``Ketu-north Rahu-south'' toggle records the memory of a
contested scholarly tradition.  The mixed-coordinate pathology records the
seam between the Ming and Qing epochs.  The \textit{b\={a}z\`{\i}}-derived position tables
...
\section{Quantitative Demonstration}\label{sec:computation}
% ~1,500 words

\subsection*{Demonstration~1: Ketu Period Analysis}

The core claim of this paper---that the Ketu of Tang-dynasty ephemerides
tracks the osculating lunar apogee rather than the descending node---rests
ultimately on numerical evidence.  Using the JPL DE431 planetary ephemeris
(accessed via Swiss Ephemeris; Meeus polynomial series for the lunar perigee and
node), we reproduce Niu Weixing's key finding and extend it over the full
62-year span of the \textit{Qiy\`ao r\`angz\=ai ju\'e} ephemeris (806--868~CE).\footnote{All
  ephemeris computations in this section use the JPL Development Ephemeris
  DE431, which provides sub-arcsecond positional accuracy for dates within
  the historical scope of this paper.  The coordinate transformations and
  period analyses are fully reproducible from any software package that
  exposes DE431 lunar apogee and node positions.}
...
ephemeris data.  Data computed from JPL~DE431 ephemeris;
\textit{cf.}~\cite{niu1995ketu}.}
\label{fig:ketu-period}
\end{figure}

\subsection*{Demonstration~2: Four-Regime Coordinate Comparison}
...
practitioner who computes Ketu as the apogee places it in the ninth house;
...
Figure~\ref{fig:ketu-period} require only standard ephemeris queries (lunar
apogee, node, and planetary longitudes from JPL~DE431) followed by well-known
coordinate transformations (ecliptic$\leftrightarrow$equatorial via the
obliquity of date; sidereal offset via precession).  The quasi-ecliptic
projection defined in Section~\ref{sec:ming} is the sole non-standard
transformation, and its derivation is given explicitly above.  All results
are therefore reproducible by any researcher with access to a DE431-based
ephemeris package~\cite{mak2014pseudoplanets,niu1995ketu}.

\subsection*{Demonstration~3: Practical Significance of the Quasi-Ecliptic System}
...
stated precisely.  The maximum deviation of a quasi-ecliptic house boundary from
...
lies within $2.19\degree$ of a house cusp in the quasi-ecliptic system could be
...
pure equatorial measurement---indicates that the quasi-ecliptic system served a
...
identity of Ketu.  Both threads exhibit the same transformation patterns,
...
\textit{Qiy\`ao r\`angz\=ai ju\'e} immediately began repurposing \textit{Ketu} as the apogee.
...
without an established equatorial infrastructure, the quasi-ecliptic system of
...
1645 (\S\ref{sec:jesuit}) returned the \textit{qizheng siyu} to an ecliptic-based
...
24\degree$ at the present epoch), and the quantitative demonstration in
...
treatment of the Indian \textit{qizheng siyu}.
...
characterizations of \textit{S\={a}nch\'{e}n t\={o}ngz\`{a}i} and \textit{Zh\`{e}ngsh\`{\i} x\={\i}ng'\`{a}n} in Sections~\ref{sec:song}
...
The quasi-ecliptic mathematical definition (\S\ref{sec:ming}) has been derived from
first principles and demonstrated computationally (\S\ref{sec:computation}), but
cross-checking the derivation against attested historical computational
examples from the \textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng} manuscript tradition would further consolidate
the result.  The computational agreement between the DE431-derived positions
and the independently reconstructed quasi-ecliptic boundaries gives
...
Finally, this paper has examined one system (\textit{qizheng siyu}) within one
...
the identity of the pseudo-planet Ketu---through four dynasties and
thirteen centuries of the Chinese \textit{qizheng siyu} system.  The trajectory spans
...
identities for Ketu (descending lunar node; osculating lunar apogee).  Six
...
not passive copying.  Each time the \textit{qizheng siyu} crossed a cultural or
...
tropical ecliptic frame with Ketu as the lunar apogee---is not a
...
contradictory landscape of modern \textit{qizheng siyu} software.  The confused
conventions, mismatched coordinate frames, and contested Ketu toggles that
...
historical transformation.  Software that computes Ketu as the descending
...
\textit{Gu\v{o}l\v{a}o x\={\i}ngz\={o}ng} original.  The disagreement between them is real, but its source
...
practitioner in a new cultural context received the \textit{qizheng siyu}, they
